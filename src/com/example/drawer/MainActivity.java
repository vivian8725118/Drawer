...
import com.example.drawer.fragment.ViewPagerListFragment;
import com.example.drawer.interfaces.OnScrollListener;
import com.example.drawer.interfaces.TabHolderScrollingContent;
import com.example.drawer.widget.PageStripViewPager;
import com.example.drawer.widget.PageStripViewPager.State;
import com.example.drawer.widget.library.SlidingTabLayout;
...
public class MainActivity extends ActionBarActivity implements OnScrollListener {
				...
				.setOnPageChangeListener(new ViewPager.SimpleOnPageChangeListener() {
					...
					public void onPageSelected(int position) {
						...
						adjustPosition(position,
								tabHolderScrollingContent.valueAt(position));
					}
					...
					public void onPageScrolled(int position,
						...
						if (positionOffsetPixels > 0) {
							int newPosition = viewPager.getState() == State.GOING_RIGHT ? position + 1
									: position;
							adjustPosition(newPosition,
									tabHolderScrollingContent
											.valueAt(newPosition));
						}
					}
					...
					private void adjustPosition(int position,
						...
						if (isTopHidden) {
							fragmentContent.adjustScroll(arr != null ? arr[0]
									: 1, arr != null ? arr[1] : scrollY);
							ViewHelper.setTranslationY(header,
									-imageview.getHeight());
						} else {
							positionInfo.clear();
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
			int visibleItemCount, int totalItemCount, int pageIdx) {
		if (pageIdx == viewPager.getCurrentItem() && !isDragging) {
			if (firstVisibleItem != 0) {
				scrollY = slidingTab.getHeight();
				if (ViewHelper.getTranslationY(header) != -imageview
						.getHeight())
					ViewHelper.setTranslationY(header, -imageview.getHeight());
				positionInfo.put(pageIdx, new int[] { firstVisibleItem,
				...
				positionInfo.put(pageIdx, new int[] { firstVisibleItem,
						firstChild.getTop() });
				int maxScrollHeight = header.getHeight()
						- slidingTab.getHeight(); // 头部滚动空间
				isTopHidden = header.getHeight() + firstChild.getTop() <= slidingTab
						.getHeight();
				...
				if (!isTopHidden) {
					...
					ViewHelper.setTranslationY(imageview,
							-firstChild.getTop() / 2);
					ViewHelper.setTranslationY(header, -header.getHeight()
							+ scrollY);
